...
import { generateCustomUUID, generateNewChatId } from '../../utils/customUUID';
...
const WorkChat = () => {
  ...
  useEffect(() => {
    ...
    const fetchProject = async () => {
      if (!userEmail) return;
      try {
        const projectResponse = await fetch("https://krishnathummae17-debug-chatbot.hf.space/get_user_project", {
          method: "POST",
          headers: { "Content-Type": "application/json" ,"Authorization": "Bearer debugmate123" },
          credentials: "include",
...
const sendMessage = async () => {
  ...
  let sessionId = currentSessionId;
  if (!sessionId) {
    // Use generateNewChatId with projectId to ensure proper chat ID incrementing
    sessionId = generateNewChatId(projectId);
    setCurrentSessionId(sessionId);
  }
  setSessionActive(true);
  ...
  try {
    const response = await fetch("https://krishnathummae17-debug-chatbot.hf.space/chat/work", {
      method: "POST",
      headers: { "Content-Type": "application/json" ,
