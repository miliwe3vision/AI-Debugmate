...
// Example: [{ email: "abc@gmail.com", roles: ["web", "uiux"] }]
        ...
        const handleSubmit = async (e) => {
            e.preventDefault();
            try {
              // Keep roles aligned with emails (index-based)
              const assigned_to_emails = teamAssignments.map(m => m.email);
              const assigned_role = teamAssignments.map(m => 
              ...
              const result = await projectDatabaseSupabase.saveProject({
                ...
                customQuestions: customQuestionsList,
                customAnswers: customAnswers,
                assigned_to_emails, // ✅ properly aligned
                assigned_role        // ✅ properly aligned
              });
